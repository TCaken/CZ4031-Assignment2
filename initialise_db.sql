...
DO $$
DECLARE 
    user_path text := 'C:\Users\Public\csvs';
    table_names text[] := ARRAY['region', 'nation', 'part', 'supplier', 'partsupp', 'customer', 'orders', 'lineitem'];
    t_name text;
BEGIN
    FOREACH t_name IN ARRAY table_names LOOP
        EXECUTE 'COPY ' || t_name || ' from ''' || user_path || '/' || t_name || '.csv'' DELIMITER ''|'' CSV';
        RAISE NOTICE 'Import completed for: %', t_name;
    END LOOP;
END $$;